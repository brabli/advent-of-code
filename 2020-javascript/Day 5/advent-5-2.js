...
function readData(err, data) {
  if (err) {
    ...
    seatIds.forEach(seatId => {
      const plusOne = seatId + 1;
      const plusTwo = seatId + 2;
      const plusOneExists = seatIds.find(id => id === plusOne);
      const plusTwoExists = seatIds.find(id => id === plusTwo);
      if (!plusOneExists && plusTwoExists) console.log("Your seat ID is " + (seatId + 1));
    })
  }
}
